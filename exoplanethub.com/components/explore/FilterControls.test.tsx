...
function renderControls(filters: Partial<FilterState> = {}) {
  ...
  const { rerender } = render(
    <FilterControls planets={PLANETS} filters={{ ...DEFAULT_FILTERS, ...filters }} onChange={onChange} />,
  );

  return {
    onChange,
    // The real page feeds the new state back through the URL; this stands in for that round trip.
    apply: (next: FilterState) =>
      rerender(<FilterControls planets={PLANETS} filters={next} onChange={onChange} />),
  };
}
